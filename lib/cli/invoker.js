...
class Invoker {
  ...
  async invoke () {
    ...
    const { reporterErrorFormat, reporterErrorLimit, summaryFileLimit, ignoreRole: ignoreRoles, attribute: attributes, noExplain } = programOpts
    ...
    if (errors && errors.length > 0) {
      ...
      const filesWithError = Object.fromEntries(
        Array.from(errorCountFiles.entries())
          .sort(([_, a], [__, b]) => b - a)
          .slice(0, summaryFileLimit)
      )
